...
function [HeadAxes, ColorbarHandle] = sab_headplot(values, arg1, varargin)
...
plotelecopt.MarkerColor= 'k'; % pink [255,20,147]/255; %'k';
  ...
  if strcmp(g.lighting,'on')
    ...
    if ~isempty(p2)
        set(p2,'DiffuseStrength',.6,'SpecularStrength',0,...
               'AmbientStrength',.5,'SpecularExponent',5)
    end;
    set(p1,'DiffuseStrength',.6,'SpecularStrength',0,...
    'AmbientStrength',.5,'SpecularExponent',5)
    lighting gouraud     % all this gives a matte reflectance
  end  
        ...
        if newElect(i,:) ~= [0 0 0]  % plot radial lines to electrode sites
            ...
            elseif opt.labelflag == 2   % plot electrode names
                ...
                line(newElect(:,1),newElect(:,2),newElect(:,3),'marker',...
                     '.','markersize',16,'color',opt.MarkerColor,'linestyle','none');
            end
        end
    end;
